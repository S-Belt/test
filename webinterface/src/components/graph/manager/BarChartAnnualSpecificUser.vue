...
export default {
    name:"BarChartAnnualSpecificUser",
    components: { Bar },
    ...
    methods: {
        ...
        async workingTimeUserSelected() {
            ...
            const start = startOfYear.format('YYYY-MM-DD');
            const end = endOfYear.format('YYYY-MM-DD');
            const apiUrl = `${baseApiUrl}/workingtimes/${this.selectedUserId}?start=${start}&end=${end}`;
            try {
